This directory contains a number of scripts for managing and interacting with
the Ping Identity Directory Server on UNIX and Linux systems.  For batch files
that may be used on Windows systems, look in the 'bat' directory.

The available scripts are listed below.  For more information about the usage
and accepted arguments for each of these scripts, invoke them with the '--help'
argument (for example, './backup --help').
...
* create-rc-script (UNIX/Linux only)
     Create an RC script to start, stop, and restart the Ping Identity
     Directory Server on UNIX-based systems.


* create-systemd-script (UNIX/Linux only)
     Create a systemd script to start and stop the Ping Identity Directory
     Server on Linux-based systems.
...
* revert-update (deployed in install root)
...
* setup (deployed in install root)
...
* uninstall (deployed in install root)
     Uninstall Ping Identity Directory Server.


* update (deployed in install root)
